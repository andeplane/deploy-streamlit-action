...
def missing_basic_capabilities(client: CogniteClient, project: str, cred_name: str) -> List[str]:
    ...
    try:
        ...
        # The inspect/token endpoint will not fail if credentials have access to another CDF project:
        accessable_projs = {p.url_name for p in token_inspect.projects}
        if project in accessable_projs:
            logger.info(f"{cred_name.title()} credentials verified towards {project=}!")
        else:
            logger.error(
                f"Given credentials did not have access to the requested CDF {project=}. "
                f"Accessable projects (if any): {accessable_projs}"
            )
            missing.append(ACL_PROJECT_LIST)
    except CogniteAPIError:
        # This ONLY fails if we are missing BOTH 'project:LIST' and 'groups:LIST':
        logger.exception("Unable to call the token/inspect endpoint:")
